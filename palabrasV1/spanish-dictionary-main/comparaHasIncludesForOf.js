...
includes de los arrays, con el método has de los Set y
con una búsqueda en arrays usando un bucle for of
*/
...
function palabrasAleatorias(palabras,cantidad,porcentajeIncluidas=1){
    const longitud = palabras.size ?? palabras.length; // funciona con Set y Array
    const resultado=[];
...
includes. Ambos métodos de búsqueda tienen una eficiencia de O(n),
aunque la implementación de includes está más optimizada.`);
console.log(`
...
convertir previamente en array en Set. La eficiencia de has
es de O(1), siendo un orden de magnitud mejor que los métodos
anteriores O(n)`);